...
class TransfuserBackbone(nn.Module):
    ...
    """
    # Special handling for I-JEPA backbone
    if self.config.backbone == "ijepa":
        # Get I-JEPA features
        image_features = self.image_encoder(image)  # (B, num_tokens, 1280)
        batch_size = image_features.shape[0]
        num_tokens = image_features.shape[1]
        feature_dim = image_features.shape[2]
        
        # Calculate target grid size - must be perfect square
        h_w = int(math.sqrt(num_tokens))
        
        # Check if we need padding to make perfect square
        if h_w * h_w < num_tokens:
            h_w += 1  # Round up to next perfect square
        
        target_tokens = h_w * h_w
        
        # Pad to perfect square if needed (e.g., 255 -> 256 tokens)
        if target_tokens > num_tokens:
            padding_needed = target_tokens - num_tokens
            pad = torch.zeros(
                batch_size, padding_needed, feature_dim,
                device=image_features.device, 
                dtype=image_features.dtype
            )
            image_features = torch.cat([image_features, pad], dim=1)
        
        # Reshape to spatial grid: (B, tokens, C) -> (B, C, H, W)
        image_features = image_features.permute(0, 2, 1).reshape(batch_size, feature_dim, h_w, h_w)
        
        # Process LiDAR through its encoder
        if lidar is not None:
            lidar_feature_maps = self.lidar_encoder(lidar)
            lidar_features = lidar_feature_maps[-1]  # Get final feature map
        else:
            # Latent mode without LiDAR
            if self.config.latent:
                lidar_features = self.lidar_latent.repeat(batch_size, 1, 1, 1)
            else:
                # Return image features only
                return None, image_features, None
        
        # Match spatial dimensions between image and LiDAR features
        image_features_resized = F.interpolate(
            image_features, 
        ...
        # Fuse using the cross-attention fusion module
        fused_features = self.fusion_module(image_features_resized, lidar_features)
        
        # Generate BEV features for semantic segmentation if needed
        bev_feature_upscale = None
        if self.config.detect_boxes or self.config.use_bev_semantic:
            bev_feature_upscale = F.interpolate(
                fused_features,
                size=(
                    self.config.lidar_resolution_height // self.config.bev_down_sample_factor,
                    self.config.lidar_resolution_width // self.config.bev_down_sample_factor
        ...
        return bev_feature_upscale, fused_features, None
    
    # Original CNN backbone path (ResNet, etc.)
    else:
        ...
        # Stem layer - skip if not a return layer
        if len(self.image_encoder.return_layers) > 4:
        ...
        # Multi-scale fusion loop for CNN backbones
        for i in range(4):
            image_features = self.forward_layer_block(image_layers, self.image_encoder.return_layers, image_features)
            lidar_features = self.forward_layer_block(lidar_layers, self.lidar_encoder.return_layers, lidar_features)
