...
class HydraConfig(TransfuserConfig):
    ...
    # New training modes
    pdm_only_training: bool = (
        False  # Skip trajectory imitation loss, only train PDM scores
    )
    weighted_confidence_inference: bool = (
        False  # Use weighted confidence for trajectory selection
    )
    continuous_ep_training: bool = (
        False  # Train EP as continuous regression instead of binary
    )
