...
from navsim.agents.gtrs_dense.hydra_features import (
    HydraFeatureBuilder,
    HydraTargetBuilder,
)
...
def hydra_kd_imi_agent_loss_dropout(
    ...
    if regression_ep or (
        hasattr(config, "continuous_ep_training") and config.continuous_ep_training
    ):
        progress_loss = F.mse_loss(
            ego_progress.sigmoid(), vocab_pdm_score["ego_progress"].to(dtype)
        )
    else:
        progress_loss = F.binary_cross_entropy_with_logits(
            ego_progress, vocab_pdm_score["ego_progress"].to(dtype)
        )
    # expansion
    if three2two:
        ddc_gt = three_to_two_classes(
    ...
    lk_loss = F.binary_cross_entropy_with_logits(
        lane_keeping, vocab_pdm_score["lane_keeping"].to(dtype)
    )
    ...
    if include_dp:
        l2_distance = (
            -((vocab[:, :, sampled_timepoints] - target_traj[:, None]) ** 2)
            / config.sigma
        )
        ...
        l2_distance = (
            -(
                (
                    vocab[:, sampled_timepoints][None].repeat(B, 1, 1, 1)
                    - target_traj[:, None]
                )
                ** 2
            )
    ...
    ttc_loss_final = (
        config.trajectory_pdm_weight["time_to_collision_within_bound"] * ttc_loss
    )
    progress_loss_final = config.trajectory_pdm_weight["ego_progress"] * progress_loss
    ddc_loss_final = (
        config.trajectory_pdm_weight["driving_direction_compliance"] * ddc_loss
    )
    ...
    # PDM-only training: skip trajectory imitation loss
    if hasattr(config, "pdm_only_training") and config.pdm_only_training:
        loss = (
            noc_loss_final
            + da_loss_final
            + ttc_loss_final
...
class GTRSAgent(AbstractAgent):
    ...
    def initialize(self) -> None:
        ...
        state_dict: Dict[str, Any] = torch.load(
            self._checkpoint_path, map_location=device
        )["state_dict"]
    ...
    def forward(self, features: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for k, v in features.items():
            if isinstance(v, torch.Tensor):
                ...
                # Handle list of tensors (e.g., status_feature)
                features_on_device[k] = [
                    t.to(device) if isinstance(t, torch.Tensor) else t for t in v
                ]
            else:
                features_on_device[k] = v
        return self.model(features_on_device)

    def evaluate_dp_proposals(
        self, features: Dict[str, torch.Tensor], dp_proposals
        ...
        for k, v in features.items():
            if isinstance(v, torch.Tensor):
                ...
                features_on_device[k] = v
        return self.model.evaluate_dp_proposals(features_on_device, dp_proposals)
    ...
    def compute_loss(
        ...
        for k in self.metrics:
            ...
            scores[k] = torch.from_numpy(np.concatenate(tmp, axis=0)).to(
                predictions["trajectory"].device
            )
    ...
    def get_optimizers(
        ...
        img_backbone_params = list(
            filter(
                lambda kv: backbone_params_name in kv[0], self.model.named_parameters()
            )
        )
        default_params = list(
            filter(
                lambda kv: backbone_params_name not in kv[0],
        ...
        if self.scheduler == "default":
            return torch.optim.Adam(
                params_lr_dict, lr=self._lr, weight_decay=self._config.weight_decay
            )
        elif self.scheduler == "cycle":
            optim = torch.optim.Adam(params_lr_dict, lr=self._lr)
            return {
                "optimizer": optim,
                "lr_scheduler": OneCycleLR(optim, max_lr=0.001, total_steps=20 * 196),
            }
        else:
