...
class HydraModel(nn.Module):
    def __init__(self, config: HydraConfig):
        ...
        self._keyval_embedding = nn.Embedding(
            config.img_vert_anchors * config.img_horz_anchors * img_num,
            config.tf_d_model,
        ...
        self.downscale_layer = nn.Conv2d(
            self._backbone.img_feat_c, config.tf_d_model, kernel_size=1
        )
        self._status_encoding = nn.Linear(
            (4 + 2 + 2) * config.num_ego_status, config.tf_d_model
        )
    ...
    def evaluate_dp_proposals(
        self, features, dp_proposals, topk=10, dp_only_inference=False
    ):
        # Handle both list and tensor formats
        status_feature_raw = features["status_feature"]
        if isinstance(status_feature_raw, list):
            # Training mode: list of 1D tensors [tensor([8])]
            # Extract and add batch dimension: [8] -> [1, 8]
            status_feature: torch.Tensor = status_feature_raw[0].unsqueeze(0)
        else:
            # Evaluation mode: already has batch dimension [1, 8]
            status_feature: torch.Tensor = status_feature_raw

        camera_feature = features["camera_feature"]

        # status_feature should now be [B, 8] where B=1 for both training and evaluation
        if self._config.num_ego_status == 1 and status_feature.shape[-1] == 8:
            # Single ego status with 8 features: [B, 8] -> [B, 256]
            status_encoding = self._status_encoding(status_feature)
        elif self._config.num_ego_status == 1 and status_feature.shape[-1] == 32:
            # Legacy path: take first 8 features
        ...
        trajectory = self._trajectory_head.eval_dp_proposals(
            keyval,
            status_encoding,
            dp_proposals,
            topk=topk,
    ...
    def forward(
        self, features: Dict[str, torch.Tensor], interpolated_traj=None
...
class HydraTrajHead(nn.Module):
    def __init__(
        ...
        self.vocab = nn.Parameter(
            torch.from_numpy(np.load(vocab_path)), requires_grad=False
        )
        ...
        if self.normalize_vocab_pos:
            self.encoder = MemoryEffTransformer(
                d_model=d_model, nhead=nhead, dim_feedforward=d_model * 4, dropout=0.0
            )
    ...
    def forward(
        self, bev_feature, status_encoding, interpolated_traj=None
    ) -> Dict[str, torch.Tensor]:
        result = {}
        # vocab: 4096, 40, 3
        # bev_feature: B, 32, C
        ...
        # Check if weighted confidence inference is enabled
        if (
            hasattr(self.config, "weighted_confidence_inference")
            and self.config.weighted_confidence_inference
        ):
            # Weighted confidence mode - use confidence weights for trajectory selection
            confidence_scores = result["imi"].softmax(
                -1
            )  # Use imitation confidence as base
            pdm_scores = (
                result["drivable_area_compliance"].sigmoid() * 0.3
                + result["no_at_fault_collisions"].sigmoid() * 0.2
                + result["time_to_collision_within_bound"].sigmoid() * 0.2
                + result["ego_progress"].sigmoid() * 0.2
                + result["lane_keeping"].sigmoid() * 0.1
            )
            scores = confidence_scores * pdm_scores  # Weight by confidence
        else:
            # Original hardcoded scoring
            scores = (
    ...
    def eval_dp_proposals(
        self,
        bev_feature,
        status_encoding,
        dp_proposals,
        topk=10,
        dp_only_inference=False,
    ) -> Dict[str, torch.Tensor]:
        # vocab: 4096, 40, 3
        # bev_feature: B, 32, C
        # embedded_vocab: B, 4096, C
        ...
        vocab = torch.cat([vocab.view(L, -1)[None].repeat(B, 1, 1), dp_proposals], 1)
        ...
        if dp_only_inference:
            ...
            result["trajectory"] = dp_proposals[
                scene_cnt_tensor, selected_indices
            ].view(B, HORIZON, 3)
        else:
            selected_indices = scores.argmax(1)
            result["trajectory"] = vocab[scene_cnt_tensor, selected_indices].view(
                B, HORIZON, 3
            )
        result["overall_scores"] = (
            1
            * result["traffic_light_compliance"].sigmoid()
            * 1
            * result["no_at_fault_collisions"].sigmoid()
            * 1
            * result["drivable_area_compliance"].sigmoid()
            * 1
            * result["driving_direction_compliance"].sigmoid()
            * (
                5.0 * result["time_to_collision_within_bound"].sigmoid()
                + 5.0 * result["ego_progress"].sigmoid()
                + 2.0 * result["lane_keeping"].sigmoid()
        ...
        _, topk_indices = torch.topk(
            result["overall_log_scores"], k=topk, dim=1
        )  # [B, 10]
        ...
        for k in self.heads.keys():
            ...
            selected_dp_scores = torch.gather(
                dp_scores, dim=1, index=topk_indices
            )  # [B, 10]
            result[k] = torch.cat(
                [vocab_scores, selected_dp_scores], dim=1
            )  # [B, 16384 + 10]
        return result
