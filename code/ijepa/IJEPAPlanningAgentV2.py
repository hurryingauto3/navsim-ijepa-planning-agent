...
class EgoFeatureBuilder(AbstractFeatureBuilder):
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            else: logger.warning(f"Builder: Unexpected command format {type(command_raw)} for current frame. Using zero vector.")
...
class IJEPAPlanningAgent(AbstractAgent, pl.LightningModule): # Inherit from LightningModule
    ...
    def compute_loss(
        ...
        # Log individual loss components using Lightning's log method
        self.log('base_loss', base_loss, on_step=True, on_epoch=True, prog_bar=False, logger=True)
        if self._accel_loss_weight > 0: # Log if the original weight was positive
