...
class IJEPAPlanningAgent(AbstractAgent):
    ...
    def __init__(
        ...
        self._mlp_weights_path_config = checkpoint_path
                ...
                try:
                    ...
                    for k, v in ckpt.items():
                        if k.startswith("agent.mlp."):
                            new_k = k.replace("agent.mlp.", "")
                            mlp_state[new_k] = v

                    if not mlp_state:
                        raise ValueError("No 'agent.mlp.' keys found in checkpoint")

                    info = self.mlp.load_state_dict(mlp_state, strict=False)
                    if info.missing_keys:
                        print(f"Warning: Missing keys during MLP load: {info.missing_keys}")
                    if info.unexpected_keys:
                        print(f"Warning: Unexpected keys during MLP load: {info.unexpected_keys}")
