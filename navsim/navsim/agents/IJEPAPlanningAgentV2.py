        ...
        if trainer.global_step == 0 and batch_idx == 0:
            ...
            try:
                ...
                if "trajectory_gt" in targets and targets["trajectory_gt"] is not None and targets["trajectory_gt"].numel() > 0:
                    ...
                    logger.info("\n[DEBUG] FIRST-BATCH  GT  (first %d poses): %s", min(3, gt_abs.shape[0]), gt_abs[:min(3, gt_abs.shape[0])]) # Print up to 3 poses
                    ...
                    with torch.no_grad():
                        ...
                        if "trajectory" in predictions and predictions["trajectory"] is not None and predictions["trajectory"].numel() > 0:
                             ...
                             logger.info("[DEBUG] FIRST-BATCH  PRED (relative, first %d): %s", min(3, pred_rel.shape[0]), pred_rel[:min(3, pred_rel.shape[0])]) # Print up to 3 poses
                             ...
                             logger.info("[DEBUG] FIRST-BATCH  PRED (ABS, first %d): %s", min(3, pred_abs.shape[0]), pred_abs[:min(3, pred_abs.shape[0])]) # Print up to 3 poses
                        else:
...
# --- Feature Builders (Kept as requested) ---
...
class EgoFeatureBuilder(AbstractFeatureBuilder):
    """
    Feature builder for extracting *current* ego status features (velocity, acceleration, driving command).
    Formats the driving command into a one-hot vector.
    Returns 8 features: [vx, vy, ax, ay, cmd0..3].
    """
    NUM_DRIVING_COMMANDS = 4
    SINGLE_FRAME_EGO_DIM = 2 + 2 + NUM_DRIVING_COMMANDS # vx, vy, ax, ay, command_one_hot

    def __init__(self): # No history frames parameter as per your request
        super().__init__()

    def get_unique_name(self) -> str:
        return "ego_features" # Keep this key name

    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        # Check if there's a current ego status (last frame)
        if not agent_input.ego_statuses or len(agent_input.ego_statuses) == 0:
            logger.warning(f"{self.get_unique_name()}: Current ego status missing. Returning zero placeholder of size {self.SINGLE_FRAME_EGO_DIM}.")
            return {self.get_unique_name(): torch.zeros(self.SINGLE_FRAME_EGO_DIM, dtype=torch.float32)}

        # Use the last ego_status in the history list (current frame at index -1)
        ...
        if velocity.shape[-1] != 2 or acceleration.shape[-1] != 2:
            logger.warning(f"Builder: Velocity or acceleration not 2D for current frame. Vel shape: {velocity.shape}, Accel shape: {acceleration.shape}. Using zeros.")
        ...
        try:
            if isinstance(command_raw, np.ndarray):
                if command_raw.size == 1:
                    ...
                    else: logger.warning(f"Builder: Invalid command index {command_index} in array for current frame.")
                elif command_raw.size == self.NUM_DRIVING_COMMANDS:
                    command_one_hot = torch.from_numpy(command_raw).float()
                else: logger.warning(f"Builder: Unexpected numpy array size {command_raw.shape} for current frame.")
            elif isinstance(command_raw, (int, float)):
                command_index = int(command_raw)
                if 0 <= command_index < self.NUM_DRIVING_COMMANDS: command_one_hot[command_index] = 1.0
                else: logger.warning(f"Builder: Invalid command index {command_index} for current frame.")
                ...
                logger.warning(f"Builder: Unexpected command format {type(command_raw)} for current frame. Using zero vector.")
                command_one_hot = torch.zeros(self.NUM_DRIVING_COMMANDS, dtype=torch.float32) # Use zero vector on unexpected format
        except Exception as e:
            logger.error(f"Error processing driving command '{command_raw}' for current frame: {e}. Using zero vector.", exc_info=True)
            command_one_hot = torch.zeros(self.NUM_DRIVING_COMMANDS, dtype=torch.float32)
        # --- End robust command handling ---

        # Concatenate features for the current frame [vel_x, vel_y, acc_x, acc_y, cmd_0..3]
        ego_features = torch.cat([velocity, acceleration, command_one_hot], dim=-1) # Shape [SINGLE_FRAME_EGO_DIM = 8]


        # Final safety check on shape
        if ego_features.shape[-1] != self.SINGLE_FRAME_EGO_DIM:
            logger.error(f"Builder: Final ego features dim mismatch {ego_features.shape[-1]} vs expected {self.SINGLE_FRAME_EGO_DIM}. Returning zero placeholder.")
            return {self.get_unique_name(): torch.zeros(self.SINGLE_FRAME_EGO_DIM, dtype=torch.float32)}

        return {self.get_unique_name(): ego_features}


# --- Target Builder (Cleaned as per previous step) ---
...
class TrajectoryTargetBuilderGT(AbstractTargetBuilder):
    ...
    """
    # Removed num_history_frames from constructor as it's not used for returning absolute targets
...
class IJEPAPlanningAgent(AbstractAgent, pl.LightningModule): # Inherit from LightningModule
    ...
    def get_optimizers(self):
        """
        Compatibility method for AbstractAgent interface. Returns the optimizer(s) as expected by NAVSIM core.
        """
    ...
    Uses single-frame ego features.
    """
    # --- Constants ---
    # NUM_FUTURE_FRAMES is now derived from trajectory_sampling
    IJEP_DIM = 1280
    # Use the dimension from the single-frame EgoFeatureBuilder
    EGO_DIM = EgoFeatureBuilder.SINGLE_FRAME_EGO_DIM # Should be 8
    HIDDEN_DIM = 256 # Using 2x256 MLP based on local Exp 5 preference
    NUM_DRIVING_COMMANDS = EgoFeatureBuilder.NUM_DRIVING_COMMANDS # Should be 4
    # --- End Constants ---
    ...
    def __init__(
        ...
        # num_history_frames is present in __init__ but not used by the single-frame EgoFeatureBuilder.
        # Keeping it for potential future compatibility if you switch builders.
        num_history_frames: int = 1, # Set default to 1 for single frame ego features
        ...
        # Store parameters
        self._num_history_frames = num_history_frames # Stored, but not used by the current EgoFeatureBuilder
        ...
        # Total input dimension for MLP: Visual features + Single-frame Ego features
        mlp_input_dim = self.IJEP_DIM + self.EGO_DIM # EGO_DIM is 8 here
        logger.info(f"MLP input dimension: {mlp_input_dim}")
        torch.set_float32_matmul_precision('medium') # Set matmul precision

        # Define MLP (Using 2x256 based on local Exp 5 preference)
        self.mlp = torch.nn.Sequential(
            torch.nn.Linear(mlp_input_dim, self.HIDDEN_DIM),
            torch.nn.ReLU(),
    ...
    def get_feature_builders(self) -> List[AbstractFeatureBuilder]:
        # Return the EgoFeatureBuilder which provides single-frame features
        return [
            CameraImageFeatureBuilder(),
            # Instantiate EgoFeatureBuilder (it uses single frame logic internally)
            EgoFeatureBuilder()
        ]

    def get_target_builders(self) -> List[AbstractTargetBuilder]:
        # Removed num_history_frames from TrajectoryTargetBuilderGT constructor call
    ...
    def forward(self, features: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            ego_features_tensor = features["ego_features"].to(device) # Using "ego_features" key
             ...
             try:
                  ...
                  batch_size = image_tensor_raw.shape[0] if 'image_tensor_raw' in locals() and image_tensor_raw is not None and image_tensor_raw.ndim >= 1 else (ego_features_tensor.shape[0] if 'ego_features_tensor' in locals() and ego_features_tensor.ndim >= 1 else next(iter(features.values())).shape[0])
             except Exception:
                  batch_size = 1 # Default to 1 if batch size cannot be determined
        ...
        try:
            ...
            # Assuming max value indicates range [0, 255], otherwise assume [0, 1]
            max_val = image_tensor_raw.max() if image_tensor_raw.numel() > 0 else 0
            if max_val > 1.0 + 1e-6: # Add small epsilon for floating point comparisons
            ...
            batch_size = image_tensor_raw.shape[0] if 'image_tensor_raw' in locals() and image_tensor_raw is not None and image_tensor_raw.ndim >= 1 else 1
            zero_preds = torch.zeros(batch_size, self._trajectory_sampling.num_poses, 3, dtype=torch.float32, device=device)
            return {"trajectory": zero_preds}
        ...
        with torch.no_grad(): # Ensure no gradients are calculated for the frozen encoder
            try:
                ...
                if visual_features is None or visual_features.shape[-1] != self.IJEP_DIM:
                    raise ValueError(f"I-JEPA feature extraction resulted in incorrect final dim {visual_features.shape[-1] if visual_features is not None else 'None'}. Expected {self.IJEP_DIM}.")
        ...
        # Predict with MLP
        try:
            # Use the agent's calculated EGO_DIM (which is 8 for single frame)
            expected_ego_dim = self.EGO_DIM
            if ego_features_tensor.shape[-1] != expected_ego_dim:
                 raise ValueError(f"Ego feature dim mismatch: {ego_features_tensor.shape[-1]} vs expected {expected_ego_dim}")
            if ego_features_tensor.shape[0] != visual_features.shape[0]:
                 raise ValueError(f"Batch size mismatch between visual features ({visual_features.shape[0]}) and ego features ({ego_features_tensor.shape[0]}).")
            ...
            batch_size = combined_features.shape[0] if 'combined_features' in locals() and combined_features is not None and combined_features.ndim >= 1 else (visual_features.shape[0] if 'visual_features' in locals() and visual_features is not None and visual_features.ndim >= 1 else 1)
            zero_preds = torch.zeros(batch_size, self._trajectory_sampling.num_poses, 3, dtype=torch.float32, device=device)
            return {"trajectory": zero_preds}
    ...
    def compute_loss(
        ...
        # Determine effective weights based on available poses for derivative calculation
        effective_accel_loss_weight = self._accel_loss_weight
        effective_jerk_loss_weight = self._jerk_loss_weight

        if num_poses < 3:
             if self._accel_loss_weight > 0 or self._jerk_loss_weight > 0:
             ...
             effective_accel_loss_weight = 0.0
             effective_jerk_loss_weight = 0.0
        elif num_poses < 4:
             if self._jerk_loss_weight > 0:
        ...
        if effective_accel_loss_weight > 0 or effective_jerk_loss_weight > 0:
            try:
                # Calculate velocity from predicted absolute poses (XY only)
                # (B, T, 3) -> (B, T-1, 2)
                # Check if we have at least 2 poses to compute velocity
                if pred_abs.shape[1] > 1:
                    pred_vel = (pred_abs[:, 1:, :2] - pred_abs[:, :-1, :2]) / self._interval_length

                    if effective_accel_loss_weight > 0 or effective_jerk_loss_weight > 0:
                         # Calculate acceleration from predicted velocity (XY only)
                         # (B, T-1, 2) -> (B, T-2, 2)
                         # Check if we have at least 2 velocity vectors to compute acceleration
                         if pred_vel.shape[1] > 1:
                             pred_accel = (pred_vel[:, 1:, :] - pred_vel[:, :-1, :]) / self._interval_length
                             if effective_accel_loss_weight > 0:
                                  # L2 norm of acceleration, averaged over batch and time dimensions
                                  # Only compute if there are acceleration points
                                  if pred_accel.numel() > 0:
                                     accel_penalty = torch.mean(torch.norm(pred_accel, dim=-1)) * effective_accel_loss_weight
                                  else:
                                      logger.warning("Predicted acceleration tensor is empty. Skipping acceleration penalty.")


                             if effective_jerk_loss_weight > 0:
                                  # Calculate jerk from predicted acceleration (XY only)
                                  # (B, T-2, 2) -> (B, T-3, 2)
                                  # Check if we have at least 2 acceleration vectors to compute jerk
                                  if pred_accel.shape[1] > 1:
                                      pred_jerk = (pred_accel[:, 1:, :] - pred_accel[:, :-1, :]) / self._interval_length
                                      # Only compute if there are jerk points
                                      if pred_jerk.numel() > 0:
                                          # L2 norm of jerk, averaged over batch and time dimensions
                                          jerk_penalty = torch.mean(torch.norm(pred_jerk, dim=-1)) * effective_jerk_loss_weight
                                      else:
                                           logger.warning(f"Predicted jerk tensor is empty. Jerk loss weight ignored.")
                                  else:
        ...
        if self._jerk_loss_weight > 0: # Log if the original weight was positive
             self.log('jerk_penalty', jerk_penalty, on_step=True, on_epoch=True, prog_bar=False, logger=True)
        self.log('total_loss', total_loss, on_step=True, on_epoch=True, prog_bar=True, logger=True) # Log total loss
    ...
    def validation_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        ...
        # Ensure MLP is in eval mode too
        self.mlp.eval()
        with torch.no_grad():
             features, targets = batch
             predictions = self.forward(features)
             loss = self.compute_loss(features, targets, predictions)
        # Set agent back to train mode (Lightning handles self.train() after validation_step_end)
        # self.train() # This call is often managed by the trainer

        # Log validation loss only at epoch end
    ...
    def get_training_callbacks(self) -> List[pl.Callback]:
        ...
        # You can add other callbacks here, e.e.g., ModelCheckpoint, EarlyStopping
        return callbacks

    # Required AbstractAgent methods (already implemented, just listed for clarity)
    # def get_sensor_config(self) -> SensorConfig: ...
    # def get_feature_builders(self) -> List[AbstractFeatureBuilder]: ...
    # def get_target_builders(self) -> List[AbstractTargetBuilder]: ...
    # def name(self) -> str: ...