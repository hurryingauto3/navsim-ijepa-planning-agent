...
from navsim.common.dataclasses import MissingSensorFileError

logger = logging.getLogger(__name__)

# Special sentinel returned when a sample should be skipped due to missing sensor files
MISSING_SAMPLE = None
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
        ...
        except MissingSensorFileError as e:
            # Log and return a sentinel so the collate_fn can filter this sample out
            logger.warning("Skipping token %s due to missing sensor files: %s", token, e)
