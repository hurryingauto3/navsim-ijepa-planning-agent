...
class MissingSensorFilesError(Exception):
    """Raised when required sensor files (images/lidars) are missing for a scene."""
    def __init__(self, missing_paths: list):
        super().__init__(f"Missing sensor files: {missing_paths}")
...
# Exceptions
class MissingSensorFileError(FileNotFoundError):
    """Raised when a required sensor file (image/lidar) is missing on disk."""
    pass
...
class Cameras:
    ...
    def from_camera_dict(
        ...
        MISSING_FILES_LOG = os.environ.get(
            "NAVSIM_MISSING_FILES_LOG", "/scratch/ah7072/experiments/missing_files.log"
        )

        def _record_missing_file(path: Union[str, Path]):
            try:
                ts = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
                with open(MISSING_FILES_LOG, "a", encoding="utf-8") as fh:
                    fh.write(f"{ts}\t{os.getpid()}\t{path}\n")
            except Exception:
                logger.debug("Failed to record missing file %s", path, exc_info=True)
        ...
        for camera_name in camera_dict.keys():
            ...
            if camera_identifier in sensor_names:
                ...
                try:
                    ...
                    logger.warning("Missing image file: %s. Recording and raising MissingSensorFileError.", image_path)
                    _record_missing_file(str(image_path))
                    raise MissingSensorFileError(f"Missing sensor image: {image_path}")
                except Exception:
                    # If any other image error occurs, record and raise MissingSensorFileError
                    logger.exception("Error loading image %s.", image_path)
                    _record_missing_file(str(image_path))
                    raise MissingSensorFileError(f"Error loading sensor image: {image_path}")
